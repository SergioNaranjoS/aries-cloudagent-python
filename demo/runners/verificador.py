...
class VerificadorAgent(AriesAgent):
    def __init__(
        self,
        ident: str,
        http_port: int,
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "presentation-received":
            ...
            is_proof_of_credencial = (
                pres_req["name"] == "Proof of Credencial"
            )
            if is_proof_of_credencial:
...
async def main(args):
    verificador_agent = await create_agent_with_args(args, ident="verificador")

    try:
        log_status(
            "#1 Provision an agent and wallet, get back configuration details"
            + (
                f" (Wallet type: {verificador_agent.wallet_type})"
                if verificador_agent.wallet_type
                else ""
            )
        )
        agent = VerificadorAgent(
            "verificador.agent",
            verificador_agent.start_port,
            verificador_agent.start_port + 1,
            genesis_data=verificador_agent.genesis_txns,
            genesis_txn_list=verificador_agent.genesis_txn_list,
            no_auto=verificador_agent.no_auto,
            tails_server_base_url=verificador_agent.tails_server_base_url,
            timing=verificador_agent.show_timing,
            multitenant=verificador_agent.multitenant,
            mediation=verificador_agent.mediation,
            wallet_type=verificador_agent.wallet_type,
            seed=verificador_agent.seed,
        )

        verificador_agent.public_did = True
        # acme_schema_name = "employee id schema"
        # acme_schema_attrs = ["employee_id", "name", "date", "position"]
        await verificador_agent.initialize(
            the_agent=agent,
        ...
        await verificador_agent.generate_invitation(display_qr=True, wait=True)
        ...
        if verificador_agent.show_timing:
            timing = await verificador_agent.agent.fetch_timing()
            if timing:
                for line in verificador_agent.agent.format_timing(timing):
                    log_msg(line)

    finally:
        terminated = await verificador_agent.terminate()
...
if __name__ == "__main__":
    parser = arg_parser(ident="verificador", port=8040)
